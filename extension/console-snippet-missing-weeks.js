...
//
// EXPLICIT-WINDOW page-console backfill for the SUMO AAQ fetcher (aaq-scraper
// #45). Like console-snippet-8weeks.js, but fetches an ARBITRARY list of
// [start, end] windows instead of N trailing weeks — for filling non-contiguous
// gaps. Downloads ONE bundle PER WINDOW so a later failure never loses the
// windows already on disk. Honors SUMO's long 429 Retry-After (~10-15 min),
// jitters between API calls, and pauses 1-4 min between windows.
//
// The CONFIG below is preset to the two weeks missing between the May 26 → Jul
// 20 desktop bundles: 2026-06-02..06-08 and 2026-06-30..07-06.
//
// HOW TO RUN
//   1. Open a support.mozilla.org tab and make sure it browses normally
//      (the Fastly challenge has passed for your session).
//   2. Keep the machine awake for the whole run:
...
//   4. Edit the CONFIG block below if needed, paste the whole file, press Enter.
//      The first download may trigger a browser "allow multiple downloads?"
//      prompt -> allow.
//   5. It downloads one aaq-<product>-<start>_<end>.json per window. Then, per file:
...
(async () => {
  ...
  const windows = [                    // explicit [start, end] windows (YYYY-MM-DD, inclusive)
  ...
  const minWeekPauseMin = 1, maxWeekPauseMin = 4; // pause between windows (minutes)
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      if (res.status === 429) {
        ...
        if (waitS !== null && waitS > max429WaitS)
          throw new Error(`HTTP 429: asked to wait ${Math.round(waitS)}s (> ${max429WaitS}s cap) - retry this window later.`);
  ...
  console.log(`[aaq] ${product}: ${windows.length} explicit window(s)`);
  const done = [], failed = [];
  for (let i = 0; i < windows.length; i++) {
    const [start, end] = windows[i];
    console.log(`[aaq] window ${i + 1}/${windows.length}: ${start} .. ${end}`);
    try {
      const r = await fetchWeek(start, end);
      done.push({ start, end, ...r });
    } catch (e) {
      console.error(`[aaq] window ${i + 1} (${start}..${end}) FAILED: ${(e && e.message) || e}`);
      failed.push({ start, end, error: String((e && e.message) || e) });
    }
    // Random 1-4 min pause between windows (not after the last).
    if (i < windows.length - 1) {
      const pauseMs = rand(minWeekPauseMin * 60, maxWeekPauseMin * 60) * 1000;
      console.log(`[aaq] pausing ${Math.round(pauseMs / 1000)}s before next window…`);
      await sleep(pauseMs);
    }
  }

  console.log(`[aaq] DONE. ${done.length}/${windows.length} window(s) downloaded.`);
  if (failed.length) {
    console.warn(`[aaq] ${failed.length} window(s) FAILED - re-run just these (edit windows[] to the failed ones):`);
